<html>
    ...
    <body>
        ...
        <div class="order_div">
            ...
            echo("<p class='order_p'>importo totale: ".$importo." €</p>");
            ?>
            ...
            <br>
            <button class="order_button" onclick="submit()">Concludi ordine</button>
            <p id="serverResponse"></p>
        </div>
        <div class="summary_div">
        <?php
            $cart_items = $conn->query("SELECT p.immagine, f.id_f, o.id_o, f.capacita, c.quantita, c.importo_parziale, p.nome FROM carrello as c, formato as f, prodotto as p, ordine as o WHERE o.stato=0 AND c.id_f = f.id_f AND f.id_p=p.id_p AND c.id_o=o.id_o AND o.id_u=".$_SESSION['id_u']);
            $cart_item = mysqli_fetch_assoc($cart_items);
            echo("<table class='summary_table'>");
            echo("<tr class='summary_tr'>");
            echo("<th class='summary_td'> </th>");
            echo("<th class='summary_td'>Prodotto</th>");
            echo("<th class='summary_td'>Formato</th>");
            echo("<th class='summary_td'>Quantità</th>");
            echo("<th class='summary_td'>Totale</th>");
            ...
            while($cart_item){
                ...
                echo("<td class='summary_td'>".$cart_item["capacita"]." ml</td>");
                echo("<td class='summary_td'>".$cart_item["quantita"]."</td>");
                echo("<td class='summary_td'>".$cart_item["importo_parziale"]." €</td>");
                echo("<td class='summary_td'><a href='remove_cart_item.php?id_o=".$cart_item["id_o"]."&id_f=".$cart_item["id_f"]."'><img style='width:20px; height:20px' src='Images/x_icon.jpg'</a></td>");
                echo("</tr>");
                $cart_item = mysqli_fetch_assoc($cart_items);
            }
            echo("</table>");
